...
$(function(){
	...
	//Home Page Changing Images

	$('.slides img:first').addClass('top');
	var change = function(){
		//declare current and next image in the slider
		var currentImg = $('.slides img.top');
		var nextImg = currentImg.next();
		//making sure slider is looping and changing classes
		if(!nextImg.length){
			nextImg = $('.slides img:first');
			nextImg.addClass('top');
			currentImg.removeClass('top');
		}else {
	...
	// the pace of slider changing images
	...
	$('#pHidden').hide(); 
	$('.pShow').on('click', function(e){
		e.preventDefault(); //prevent default event
		//show p and change p class to 'show' if its 'hide'
		if($('#pHidden').hasClass('hide')){
		 	$('#pHidden').show();
		 	$('#pHidden').removeClass('hide');
		 	$('#pHidden').addClass('show');
		//hide p and change p class to 'hide' if its 'show'
		}else{
			$('#pHidden').hide();
			$('#pHidden').removeClass('show');
		 	$('#pHidden').addClass('hide');
		}
	});
	...
	var items = '';
	// hide list items
	$('li.hot').hide().each(function(index) {       
		// then fade them in   
    	$(this).delay(450 * index).fadeIn(1600);     
  	});	

  	//tools page checklist counter
  	function updateCount() {   
  		//number of items in the list that has class complete                    
    	var items = $('li.hot[class!=complete]').length; 
    	//put them into the span
    	$('#counter').text(items);                   
 	}
  	updateCount(); 

 	//tools page list in checklist changing classes
    ...
    //Instruction Page Steps Slider

    //declare variables
  	var counterDiv = 1;
  	var $countDivs = $('.countDivs');
  	//put the update count of div in span
  	$countDivs.text(counterDiv);

  	//instruction Page Steps Div Counter
    ...
    //declare variables
    var currentDiv = $('.activate');
    var nextDiv = currentDiv.next();
    //keep incrementing and change div if div count is less than 7
    if(counterDiv < 7){
      currentDiv.removeClass('activate').css('z-index', -10);
      nextDiv.addClass('activate').css('z-index', 10);
      counterDiv += 1;
      updateCountDiv();
    }
  	});
    ...
    //keep decrementing and change div if div count is more than 1
    if(counterDiv > 1){
      currentDiv.removeClass('activate').css('z-index', -10);
      prevDiv.addClass('activate').css('z-index', 10);
      counterDiv -= 1;
      updateCountDiv();
    }
  }); 

	//Thank Us page Contact Form
